...
function EditJob() {
  ...
  useEffect(() => {
    fetch(`${process.env.REACT_APP_API}/jobs/${id}`)
      ...
      .then(data => {
        ...
        if (data.custom_requirements && data.custom_requirements.length > 0) {
          setCustomReqs(data.custom_requirements)
        }
      })
  }, [id])
  ...
  const WeightInput = ({ label, weightKey }) => (
    <div style={{
      ...
      <span style={{ fontSize: "13px", color: "#666", minWidth: "80px" }}>{label}:</span>
      <input
        type="number"
        min="0"
        max="100"
                  ...
                  <div key={i} style={{
                    ...
                    <div style={{ marginBottom: "10px" }}>
                      ...
                      <select
                        ...
                        style={{ padding: "6px 10px", border: "1.5px solid #ddd", borderRadius: "8px", fontSize: "13px", background: "white" }}
                      >
                        {req.options.map((opt, j) => (
